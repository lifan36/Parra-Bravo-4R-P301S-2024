...
# Venn Diagram for 4R/P301S+K18 bulk RNA-seq overlap with AD brain
...
#P301S+K18/4R+K18 vs AD_AT8+/AD_AT8- up and down
{
  P301S_dn<-read.table(
    file = "P301S_K18vs4R_K18_SigDown.csv", 
    header = TRUE, sep =",")
  AT8_dn<-read.table(file = "Ex_AT8_vs_MAP2_SigDown.csv", header = TRUE, sep = ",")
  AT8_dn <- subset(AT8_dn, p_val_adj < 0.05)
  P301S_up<-read.table(
    file = "P301S_K18vs4R_K18_SigUp.csv", 
    header = TRUE, sep =",")
  ...
  names(vd)<-c("P301S+K18 vs 4R+K18","AT8+ vs AT8-", "P301S+K18 vs 4R+K18","AT8+ vs AT8-")
  
  
  #for list of overlapping genes
  overlap_dn<-intersect(P301S_dn$Gene, AT8_dn$Gene)
  overlap_dn<-data.frame(overlap_dn)
  write.csv(overlap_dn, "overlap_dn.txt")
  overlap_up<-intersect(P301S_up$Gene, AT8_up$Gene)
  overlap_up<-data.frame(overlap_up)
  write.csv(overlap_up, "overlap_up.txt")
  
  #global (multi-set plots)
  global<-ggVennDiagram(vd[1:4])
  
  #pairwise ggplots
  P301S_K18vs4R_K18__AT8pos_AT8neg_Down<-(ggVennDiagram(c(vd[1],vd[2]))+ scale_color_brewer(palette = "Paired")) 
  P301S_K18vs4R_K18__AT8pos_AT8neg_Up<-(ggVennDiagram(c(vd[3],vd[4]))+ scale_color_brewer(palette = "Paired")) 
  
  #For Figure
  venn.diagram(list(A = 1:641, B = 613:774), fill = c("skyblue", "darkblue"), 
               alpha = c(0.5, 0.5), lwd =0, ext.text = FALSE, "venn_diagram.tiff")

  GSEA_dn <- read.csv("overlap_dn_GSEA.csv",header=T)
  ...
  ggplot(data=GSEA_dn, aes(x=name  , y=logP)) +
    ...
    ggtitle("P301S+K18 vs 4R+K18/AT8+ vs. AT8- - Down")
  ...
  #Down
  P301S_dn.list<-read.table("P301S_K18vs4R_K18_SigDown.txt")
  ...
  #Up
  P301S_up.list<-read.table("P301S_K18vs4R_K18_SigUp.txt")
...
#P301S+K18/4R+K18 vs AD_AT8+/non-AD up and down
{
  P301S_dn<-read.table(
    file = "P301S_K18vs4R_K18_SigDown.csv", 
    header = TRUE, sep =",")
  ...
  names(vd)<-c("P301S+K18 vs 4R+K18","AT8+ vs non-AD", "P301S+K18 vs 4R+K18","AT8+ vs non-AD")
  ...
  #pairwise ggplots
  P301S_K18vs4R_K18__AT8pos_nonAD_Down<-(ggVennDiagram(c(vd[1],vd[2]))+ scale_color_brewer(palette = "Paired")) 
  P301S_K18vs4R_K18__AT8pos_nonAD_Up<-(ggVennDiagram(c(vd[3],vd[4]))+ scale_color_brewer(palette = "Paired")) 
